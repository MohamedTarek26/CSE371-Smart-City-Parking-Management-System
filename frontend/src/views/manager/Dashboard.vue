...
const handleSearch = () => {
  ...
  router.push(`/manager/search?location=${searchQuery.value}`)
}

// Navigation handler
const navigateToSection = (section) => {
  router.push(`/manager/${section}`)
...
onMounted(async () => {
  ...
  if (loadUserRoleId() !== 1 || loadUserRoleId() !== 2) {
    alert("You are not authorized to access this page")
    goToSignIn()
    return
  }
  console.log('Dashboard page mounted for managers with user role:', loadUserRoleId())
})
</script>
    ...
    <header class="bg-white shadow-sm fixed w-full top-0 z-50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center justify-between">
          ...
          <!-- Logo -->
          <h1 class="text-xl md:text-2xl font-bold text-gray-900">Manager Page</h1>
