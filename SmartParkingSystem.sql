...
INSERT INTO Reservation (user_id, spot_id, reservation_time, start_time, end_time, status, penalty)
SELECT 
    ...
    @reservation_time := NOW() - INTERVAL FLOOR(RAND() * 30) DAY AS reservation_time,
...
-- Time Limits for Reservations
DELIMITER //
CREATE TRIGGER CheckReservationTimeLimit
BEFORE INSERT ON Reservation
FOR EACH ROW
BEGIN
    DECLARE time_limit INT DEFAULT 15; -- 15 minutes limit

    -- Check if the reservation time exceeds the time limit
    IF TIMESTAMPDIFF(MINUTE, NEW.reservation_time, CURRENT_TIMESTAMP) > time_limit THEN
        SET NEW.status = 'Expired'; -- Automatically expire reservations after time limit
    END IF;
END //
DELIMITER;
...
-- Test
-- Insert reservation
INSERT INTO Reservation (reservation_id, user_id, spot_id, reservation_time, start_time, end_time, status)
VALUES (27, 104, 3, NOW(), NOW() + INTERVAL 1 HOUR, NOW() + INTERVAL 2 HOUR, 'Reserved');

-- After time limit or expiration, check if the status is 'Expired'
SELECT * FROM Reservation WHERE reservation_id = 27;
...
-- INSERT INTO Reservation (reservation_id, user_id, spot_id, start_time, end_time, status)
-- VALUES (26, 10, 936, '2024-12-28 00:37:57', '2024-12-29 15:00:00', 'Reserved');
-- select * from reservation